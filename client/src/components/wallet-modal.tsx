...
export default function WalletModal({ isOpen, onClose }: WalletModalProps) {
  const { connectWithDemoWallet } = useWallet();
  const { toast } = useToast();
  const [fakeTxHash, setFakeTxHash] = useState("");
  const [showDemoConnect, setShowDemoConnect] = useState(false);

  const handleWalletConnectClick = async () => {
    // Show the demo connection option since the real WalletConnect doesn't work well in Replit
    setShowDemoConnect(true);
  };
  ...
  const handleDemoConnect = async () => {
    try {
      ...
      await connectWithDemoWallet(fakeTxHash);
      toast({
      ...
      <DialogContent className="bg-zinc-800 border-gray-700 text-white max-w-md">
        <DialogHeader>
          <div className="flex items-center justify-between">
            <DialogTitle>{showDemoConnect ? 'Enter Wallet Address' : 'Connect Wallet'}</DialogTitle>
          ...
          <DialogDescription className="text-gray-400">
            {showDemoConnect 
              ? 'Enter your wallet address to connect in demo mode'
            ...
            <div className="mb-4">
              ...
              <div className="mt-1">
                <input
                  ...
                  value={fakeTxHash}
                  onChange={(e) => setFakeTxHash(e.target.value)}
                />
              </div>
              <p className="mt-2 text-sm text-gray-400">
                Enter any text to simulate a wallet address for the demo
              </p>
            </div>
            
            <div className="flex space-x-3">
              <button
                className="flex-1 px-3 py-2 rounded-md bg-gray-700 text-white hover:bg-gray-600 transition"
                onClick={() => setShowDemoConnect(false)}
              >
                Back
              </button>
              <button
                className="flex-1 px-3 py-2 rounded-md bg-[#ff3e00] text-white hover:bg-opacity-90 transition"
